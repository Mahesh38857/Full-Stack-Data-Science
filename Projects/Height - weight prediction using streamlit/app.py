...
height_input = st.number_input("Enter the height in feet:", value=default_height, min_value=1.0)

# Predict button
if st.button('Predict'):
    # Reshape the input height to match the shape expected by the model (2D array)
    height_input_2d = np.array([[height_input]])  # Safe 2D format

    # Use the loaded model to make predictions
    predicted_weight = loaded_model.predict(height_input_2d)

    # Print the predicted weight
    st.markdown(f'<p class="prediction">Predicted weight: {predicted_weight[0]} kg</p>', unsafe_allow_html=True)


# For runing this file "python -m streamlit run app.py"
# for running streamlit file syntax will be "python -m streamlit run filename.py"
